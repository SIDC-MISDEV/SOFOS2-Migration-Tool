    ...
    public class PurchaseRequestController
    {
        ...
        public List<PurchaseRequest> GetPRHeader(string date)
        {
            try
            {
                ...
                using (var conn = new MySQLHelper(Global.SourceDatabase, PurchasingQuery.GetPRQuery(PR.PRHeader), prm))
                {
        ...
        public List<PurchaseRequestItem> GetPRItem(string date)
        {
            try
            {
                ...
                using (var conn = new MySQLHelper(Global.SourceDatabase, PurchasingQuery.GetPRQuery(PR.PRDetail), prm))
                {
                    using (var dr = conn.MySQLReader())
                    {
                        while (dr.Read())
                ...
                using (var conn = new MySQLHelper(Global.DestinationDatabase))
                {
                    ...
                    foreach (var item in _header)
                    {
                        ...
                        foreach (var detail in details)
                        {
                            ...
                            conn.ArgSQLCommand = PurchasingQuery.InsertPR(PR.PRDetail);
